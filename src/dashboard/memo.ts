...
function memoFolder(baseDirectory = DASHBOARD_FOLDER): string {
  return `${baseDirectory}/Memos`;
}

export function memoPathFor(sourcePath: string, baseDirectory = DASHBOARD_FOLDER): string {
  return normalizePath(`${memoFolder(baseDirectory)}/${sanitizeMemoBaseName(sourcePath)}.md`);
}

async function resolveMemoFilePath(app: App, sourcePath: string, baseDirectory = DASHBOARD_FOLDER): Promise<string> {
  const folder = memoFolder(baseDirectory);
  const baseName = sanitizeMemoBaseName(sourcePath);
  for (let counter = 0; ; counter++) {
    const candidateName = counter === 0 ? baseName : `${baseName} (${counter})`;
    const candidatePath = normalizePath(`${folder}/${candidateName}.md`);
...
export async function readMemos(app: App, sourcePath: string, baseDirectory = DASHBOARD_FOLDER): Promise<DocumentMemo[]> {
  const path = await resolveMemoFilePath(app, sourcePath, baseDirectory);
...
export async function writeMemos(app: App, sourcePath: string, memos: DocumentMemo[], baseDirectory = DASHBOARD_FOLDER): Promise<void> {
  const folder = memoFolder(baseDirectory);
...
export async function listMemoFiles(app: App, baseDirectory = DASHBOARD_FOLDER): Promise<Array<{ file: TFile; source: string; memos: DocumentMemo[] }>> {
  const prefix = `${memoFolder(baseDirectory)}/`;
