 ...
 */
export function dashboardPath(name: string, baseDirectory = DASHBOARD_FOLDER): string {
  return `${baseDirectory}/${name}${DASHBOARD_EXT}`;
}
...
export async function migrateDashboardKanbanWidgetsToFiles(
  ...
  for (const widget of data.widgets) {
    ...
    const path = await createKanbanFileFromConfig(vault, config, widget.id || "Board", baseDirectory);
