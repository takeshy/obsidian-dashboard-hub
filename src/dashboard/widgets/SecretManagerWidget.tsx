...
const sessionPassword = (): string => cryptoCache.getPassword() ?? "";
...
export default function SecretManagerWidget({ config, ctx }: { config: unknown; ctx?: WidgetContext }) {
  ...
  const [createPassword, setCreatePassword] = useState(sessionPassword);
  ...
  const createSecret = useCallback(async (event: FormEvent) => {
    ...
    try {
      ...
      const { encryptedPrivateKey, salt } = await encryptPrivateKey(privateKey, createPassword);
      ...
      cryptoCache.setPassword(createPassword);
  ...
  }, [ctx, createPassword, draft, folder, refresh]);
...
function SecretViewDialog({
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        ...
        if (cachedPassword) {
          try {
            const plain = await decryptFileContent(content, cachedPassword);
  ...
  const save = async (event: FormEvent) => {
    ...
    try {
      const { publicKey, privateKey } = await generateKeyPair();
      const { encryptedPrivateKey, salt } = await encryptPrivateKey(privateKey, currentPassword);
      const content = await encryptPlaintextFileContent(draft.value, publicKey, encryptedPrivateKey, salt, {
