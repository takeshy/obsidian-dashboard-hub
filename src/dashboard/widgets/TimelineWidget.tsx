...
import { DASHBOARD_FOLDER, type WidgetContext } from "../types";
...
function timelineDir(name: string, baseDirectory = DASHBOARD_FOLDER): string {
  return `${baseDirectory}/Timeline/${sanitizeName(name)}`;
}
...
function dayFilePath(name: string, date: Date, baseDirectory = DASHBOARD_FOLDER): string {
  return `${timelineDir(name, baseDirectory)}/${dateKey(date)}.md`;
}
...
async function loadTimelineFiles(ctx: WidgetContext, name: string, limit: number, filters: TimelineFilters): Promise<{ posts: TimelinePost[]; hasMore: boolean }> {
  const prefix = `${timelineDir(name, ctx.plugin.settings.baseDirectory)}/`;
...
export default function TimelineWidget({
  ...
  const baseDirectory = ctx?.plugin.settings.baseDirectory ?? DASHBOARD_FOLDER;
  ...
  useEffect(() => {
    ...
    const refs = [
      ctx.app.vault.on("create", (file) => {
        if (file.path.startsWith(`${timelineDir(name, baseDirectory)}/`)) void refresh();
      }),
  ...
  const savePostImage = useCallback(
    async (date: Date, postId: string, file: File, index: number): Promise<string> => {
      ...
      const folder = `${timelineDir(name, baseDirectory)}/attachments/${dateKey(date)}`;
  ...
  const submitPost = async () => {
    ...
    try {
      const now = new Date();
      const path = dayFilePath(name, now, baseDirectory);
      await ensureVaultFolder(ctx.app.vault, timelineDir(name, baseDirectory));
  ...
  const openAiRewrite = (target: "draft" | "edit") => {
    ...
    new AiGenerationModal(ctx.app, {
      ...
      description: "Describe how the Timeline text should be rewritten.",
      original: content,
      generate: (request) => ctx.plugin.rewriteText({
