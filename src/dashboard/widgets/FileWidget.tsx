...
function HtmlFrame({
  ...
  onMemoPointerLeave?: () => void;
  onMemoClick?: (clientX: number, clientY: number, win: Window) => boolean;
}) {
  ...
  useEffect(() => {
    ...
    // Native hash navigation from a sandboxed srcdoc iframe can replace the
    ...
    const onClick = (event: MouseEvent) => {
      ...
      try {
        ...
        // Keep the literal fragment when an EPUB contains malformed escaping.
      }
    ...
    const onPointerMove = (event: PointerEvent) => {
      const frameWin = doc.defaultView;
      if (!frameWin || event.pointerType === "touch") return;
      const frameRect = frameRef.current?.getBoundingClientRect();
      const hit = onMemoPointerMove?.(
        event.clientX,
        event.clientY,
        frameWin,
    ...
    doc.documentElement.addEventListener("pointerleave", onPointerOut);
    ...
    return () => {
      ...
      doc.documentElement.removeEventListener("pointerleave", onPointerOut);
      if (Platform.isMobile) {
        window.clearTimeout(timer);
        doc.removeEventListener("selectionchange", onSelectionChange);
      }
    };
  }, [anchorKind, html, loadTick, onMemoClick, onMemoPointerLeave, onMemoPointerMove, onSelectionClear, onSelectionContextMenu]);
...
function SelectionMenu({
  ...
  const inner = (
    ...
    >
      ...
      {showAskAi && <button type="button" onClick={onAskAi}>
        <Bot size={13} />
        {t("memo.askAi")}
      </button>}
...
function MemoPanel({
  ...
  useEffect(() => {
    ...
    const item = listRef.current?.querySelector<HTMLElement>(`[data-memo-id="${CSS.escape(focusMemoId)}"]`);
    item?.scrollIntoView({ block: "center", behavior: "smooth" });
  }, [focusMemoId]);

  const recordMemoActivity = async (action: "created" | "updated" | "deleted", memo: DocumentMemo) => {
    const content = [memo.text.trim(), memo.quote?.trim() ? `> ${memo.quote.trim().replace(/\r?\n/g, "\n> ")}` : ""]
      .filter(Boolean).join("\n\n");
    try {
      ...
      // The memo is the primary data. Keep it saved even if its activity log
      // cannot be written, and leave a useful diagnostic for desktop users.
      console.error("Memo: failed to record Timeline activity", error);
    }
  };
  ...
  const addMemo = async () => {
    ...
    try {
      ...
      await writeMemos(ctx.app, sourcePath, next, ctx.plugin.settings.baseDirectory);
      await recordMemoActivity("created", created);
      setDraft("");
      onClearSelectedQuote();
      onMemosChange(next);
  ...
  const saveEdit = async () => {
    ...
    try {
      await writeMemos(ctx.app, sourcePath, next, ctx.plugin.settings.baseDirectory);
      const updated = next.find((memo) => memo.id === editingId);
      if (updated) await recordMemoActivity("updated", updated);
      onMemosChange(next);
      cancelEdit();
    } catch (e) {
  ...
  const deleteMemo = async (memoId: string) => {
    ...
    const deleted = memos.find((memo) => memo.id === memoId);
    const next = memos.filter((memo) => memo.id !== memoId);
    try {
      await writeMemos(ctx.app, sourcePath, next, ctx.plugin.settings.baseDirectory);
      if (deleted) await recordMemoActivity("deleted", deleted);
    } catch (e) {
  ...
  const askAiAboutMemoFile = () => {
    const memoPath = memoPathFor(sourcePath, ctx.plugin.settings.baseDirectory);
    void ctx.plugin.openChatWithDraft(
      `Memo file:\n${memoPath}\n\nSource file:\n${sourcePath}`,
    );
  };

  const askAiAboutMemo = (memo: DocumentMemo) => {
    const quotation = memo.quote
      ? [
          "Quote information:",
          memo.quoteAnchor ? `Anchor: ${memo.quoteAnchor}` : "",
          memo.quotePrefix ? `Before: ${memo.quotePrefix}` : "",
          `Quote:\n${memo.quote}`,
          memo.quoteSuffix ? `After: ${memo.quoteSuffix}` : "",
        ].filter(Boolean).join("\n")
      : "";
    void ctx.plugin.openChatWithDraft([
      `Source file:\n${sourcePath}`,
...
export default function FileWidget({
  ...
  const [markdownRenderTick, setMarkdownRenderTick] = useState(0);
  ...
  useEffect(() => {
    ...
    void MarkdownRenderer.render(app, content, el, filePath, component).then(() => {
      ...
      setMarkdownRenderTick((value) => value + 1);
    });
    return () => {
      cancelled = true;
      component.unload();
  ...
  const reloadMemos = useCallback(() => {
    ...
    void readMemos(ctx.app, path, ctx.plugin.settings.baseDirectory).then(setMemos);
  }, [ctx, path]);
  ...
  const findQuoteRange = useCallback((quote: string, anchor?: QuoteAnchorData): { range: Range; win: Window } | null => {
    for (const candidate of rootsForQuotes()) {
      const root = (() => {
        ...
        if (page) return rootEl.querySelector(`[data-pdf-page="${page[1]}"], [data-page-number="${page[1]}"]`) ?? rootEl;
    ...
    const timer = window.setTimeout(() => {
      ...
      if (showSavedHighlights) {
        for (const memo of memos ?? []) {
          if (!memo.quote) continue;
          const anchor = anchorDataFromMemo(memo);
          const key = JSON.stringify([memo.quote, anchor]);
          const group = groupedMemos.get(key);
          if (group) group.memos.push(memo);
          else groupedMemos.set(key, { quote: memo.quote, anchor, memos: [memo] });
        }
      }
      const memoRanges: MemoRange[] = [];

      for (const { quote, anchor, memos: groupMemos } of groupedMemos.values()) {
  ...
  const hitMemo = useCallback((clientX: number, clientY: number, selectionWin: Window): MemoRange | null => {
    for (const group of memoRangesRef.current) {
      if (group.win !== selectionWin) continue;
      for (const rect of Array.from(group.range.getClientRects())) {
        if (
  ...
  const showMemoHover = useCallback((clientX: number, clientY: number, selectionWin: Window, hostX = clientX, hostY = clientY): boolean => {
    const group = hitMemo(clientX, clientY, selectionWin);
    if (!group) {
      setMemoHover(null);
      return false;
    }
    const latest = group.memos[group.memos.length - 1];
    const text = latest?.text.trim() || latest?.quote?.trim() || "";
    setMemoHover({
      x: Math.max(8, Math.min(hostX + 12, window.innerWidth - 332)),
      y: Math.max(8, Math.min(hostY + 16, window.innerHeight - 120)),
      text,
  ...
  const openMemoAtPoint = useCallback((clientX: number, clientY: number, selectionWin: Window): boolean => {
    ...
    setMemoHover(null);
    setFocusMemoId(undefined);
    updateConfig({ memoPanelOpen: true, memoPanelCollapsed: false });
    window.requestAnimationFrame(() => setFocusMemoId(latest.id));
    return true;
  }, [cfg, ctx, hitMemo]);
  ...
  const handleHostMemoClick = useCallback((event: React.MouseEvent<HTMLDivElement>) => {
    if (kind !== "pdf" && kind !== "markdown") return;
    if (!openMemoAtPoint(event.clientX, event.clientY, window)) return;
    event.preventDefault();
    event.stopPropagation();
  ...
  const body = () => {
    ...
    if (kind === "epub") {
      ...
      return <HtmlFrame
        ...
        onMemoPointerMove={showMemoHover}
        onMemoPointerLeave={() => setMemoHover(null)}
        onMemoClick={openMemoAtPoint}
        frameRef={frameRef}
        onLoad={() => setFrameLoadTick((value) => value + 1)}
      />;
    }
    if (kind === "image") {
      return (
    ...
    <div className="llm-hub-db-file-wrap">
      ...
      {selectionMenu && (
        <SelectionMenu
          ...
          showAskAi={ctx.plugin.hasCapability("chat")}
