...
export default function WorkflowWidget({
  ...
  const workflowAvailable = plugin?.hasCapability("workflow") ?? false;
  ...
  useEffect(() => {
    ...
    void (async () => {
      setLoading(true);
      const cached = await loadWidgetCache(app, dashboardPath, widgetId, baseDirectory);
      if (cancelled) return;
      setRecord(cached);
      setLoading(false);
    })();
  ...
  }, [app, widgetId, dashboardPath, baseDirectory]);
  ...
  const executeWorkflow = useCallback(async () => {
    if (!app || !plugin || !workflowAvailable || !widgetId || !dashboardPath || executing) return;
    ...
    try {
      ...
      await saveWidgetCache(app, dashboardPath, widgetId, next, baseDirectory);
      setRecord(next);
    } catch (err) {
      if (abort.signal.aborted) return;
  ...
  }, [app, plugin, workflowAvailable, widgetId, dashboardPath, baseDirectory, executing, cfg.workflow, cfg.outputVariable]);
