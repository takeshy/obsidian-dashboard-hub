...
describe("memoPathFor", () => {
  ...
  it("uses the configured base directory", () => {
    expect(memoPathFor("Books/sample.epub", "Workspace")).toBe("Workspace/Memos/Books_sample.epub.md");
  });
...
describe("read/writeMemos collision handling", () => {
  it("reads and writes below the configured base directory", async () => {
    const { app, files } = makeApp();
    const values = [{ id: "1", createdAt: "2026-07-04T12:00:00.000Z", text: "custom" }];
    await writeMemos(app, "Books/sample.epub", values, "Workspace");

    expect([...files.keys()]).toEqual(["Workspace/Memos/Books_sample.epub.md"]);
    await expect(readMemos(app, "Books/sample.epub", "Workspace")).resolves.toEqual(values);
  });
