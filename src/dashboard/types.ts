...
export function normalizeBaseDirectory(value: unknown): string {
  const normalized = (typeof value === "string" ? value : "")
    ...
    .join("/");
  return normalized || DASHBOARD_FOLDER;
}

export function dashboardSubfolder(baseDirectory: unknown, child: string): string {
  return `${normalizeBaseDirectory(baseDirectory)}/${child}`;
}
