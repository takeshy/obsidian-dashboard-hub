...
export function DashboardEditor({
  ...
  useEffect(() => {
    ...
    const ref = workspace.on("dashboard-hub:base-directory-changed", () => {
      setBaseDirectoryRevision((revision) => revision + 1);
    });
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const migrated = await migrateDashboardKanbanWidgetsToFiles(
          plugin.app.vault,
          initial,
  ...
  }, [plugin.app.vault, plugin.settings.baseDirectory, initial, onYamlChange]);
